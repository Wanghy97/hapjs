...
public class Network extends CallbackHybridFeature {
    ...
    protected static final String KEY_SIGNALSTRENGTH = "signalStrength";
    ...
    private int getMobileSignalStrengthFromCellInfo(Context context, TelephonyManager telephonyManager) {
        int mSignalStrength = ERROR_SIGNAL_STRENGTH;
        if (ActivityCompat.checkSelfPermission(context, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
            Log.d(TAG, "return default signalStrength because no permission.");
        ...
        if (cellInfoList != null && cellInfoList.size() > 0) {
            for (CellInfo cellInfo : cellInfoList) {
                if (cellInfo.isRegistered()) {
                    if (cellInfo instanceof CellInfoGsm) {
                        ...
                        mSignalStrength = cellSignalStrengthCdma.getDbm();
            ...
            Log.d(TAG, "return default signalStrength because cellInfoList is empty.");
            mSignalStrength = ERROR_SIGNAL_STRENGTH_NO_CELL_INFO;
        }
    ...
    private int getMobileSignalStrengthFromSignalStrength(SignalStrength signalStrength) {
        try {
            ...
            Log.d(TAG,"return default signalStrength because reflection getDbm failed.Exception:"+e.getMessage());
        }
        return ERROR_SIGNAL_STRENGTH;
    }
    ...
    private Response makeResponse(boolean metered, int type, int signalStrength) throws JSONException {
        ...
        result.put(KEY_SIGNALSTRENGTH, signalStrength);
